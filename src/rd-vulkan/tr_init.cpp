...
{
	//FIXME: lol badness
	sp_language = ri.Cvar_Get("sp_language", "english", CVAR_ARCHIVE | CVAR_NORESTART );
	//
	...
	r_surfaceWeather					= ri.Cvar_Get( "r_surfaceWeather",					"0",						CVAR_TEMP|CVAR_LATCH );
...
#endif																																		// because that is how they have been used by maps and serverside mods for
...
void RE_UpdateGLConfig( glconfig_t *glconfigOut ) {
	...
	ri.WIN_UpdateGLConfig( &glConfig );

	// todo ?
	//if (oldWidth != glConfig.vidWidth || oldHeight != glConfig.vidHeight) {
		// vulkan handles this diffrently
		// R_SyncRenderThread();
		//R_UpdateImages();
	//}
...
Q_EXPORT refexport_t* QDECL GetRefAPI( int apiVersion, refimport_t *rimp ) {
	...
	// G2API
	re.G2API_RicksCrazyOnServer				= G2API_RicksCrazyOnServer;
	re.G2API_FixGhoul2InfoLeaks				= G2API_FixGhoul2InfoLeaks;
	...
	re.G2API_GetMaxModelIndex				= G2API_GetMaxModelIndex;
